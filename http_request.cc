...
#include "tensorflow/core/platform/s3cloud/http_request.h"
...
namespace S3 {
  ...
  Status HttpRequest::Init() {
    ...
    //libcurl_->curl_easy_setopt(curl_, CURLOPT_VERBOSE,1);
  ...
  Status HttpRequest::AddS3AuthBearerHeader(const string& key,
                                        const string&signedValue) {
    TF_RETURN_IF_ERROR(CheckInitialized());
    TF_RETURN_IF_ERROR(CheckNotSent());
    return AddHeader("Authorization", strings::StrCat("AWS ", key,":",
                                                      signedValue));
  }
  ...
  Status HttpRequest::SetHeadRequest() {
    TF_RETURN_IF_ERROR(CheckInitialized());
    TF_RETURN_IF_ERROR(CheckNotSent());
    TF_RETURN_IF_ERROR(CheckMethodNotSet());
    is_method_set_ = true;
    libcurl_->curl_easy_setopt(curl_, CURLOPT_CUSTOMREQUEST, "HEAD");
    libcurl_->curl_easy_setopt(curl_, CURLOPT_NOBODY, 1L);
    return Status::OK();
  }

  Status HttpRequest::SetPutRequest() {
    TF_RETURN_IF_ERROR(CheckInitialized());
    TF_RETURN_IF_ERROR(CheckNotSent());
    TF_RETURN_IF_ERROR(CheckMethodNotSet());
    is_method_set_ = true;
    libcurl_->curl_easy_setopt(curl_, CURLOPT_CUSTOMREQUEST, "PUT");
    libcurl_->curl_easy_setopt(curl_, CURLOPT_NOBODY, 1L);
    curl_headers_ = libcurl_->curl_slist_append
      (curl_headers_, strings::StrCat("Transfer-Encoding: ").c_str());

    return Status::OK();
  }

  Status HttpRequest::SetPutFromFile(const string& body_filepath, size_t offset,size_t size) {
    TF_RETURN_IF_ERROR(CheckInitialized());
    TF_RETURN_IF_ERROR(CheckNotSent());
    TF_RETURN_IF_ERROR(CheckMethodNotSet());
    is_method_set_ = true;
    ...
    const auto fsize = ftell(put_body_) - offset;
    if(size<=0){
      size = fsize;
    }
    if(size>fsize){
      return errors::InvalidArgument
        ("filename:",body_filepath
         ," Put Size:",size," large than file size:",fsize);
    }
  ...
  size_t HttpRequest::PutCallback(void* ptr, size_t size, size_t nmemb,
                                  FILE* this_object) {
    CHECK(ptr);
    auto that = reinterpret_cast<HttpRequest*>(this_object);
    CHECK(that->put_body_);
    size_t remainedBody = that->put_size_ - that->put_body_read_;
    auto readSize = remainedBody;
    if(remainedBody<=0)return 0;
    if(remainedBody >= size*nmemb){
      readSize = size*nmemb;
    }
    size_t num = fread(ptr,1,readSize,that->put_body_);
    that->put_body_read_+=num;
    return num;
  }
