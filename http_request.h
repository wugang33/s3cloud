...
class HttpRequest {
 public:
  ...
  /// Sets the 'Authorization' header to the value of 'Bearer ' + auth_token.
  virtual Status AddS3AuthBearerHeader(const string& key,
  ...
  virtual Status SetHeadRequest();
  /// Makes the request a PUT request.
  virtual Status SetPutRequest();

  /// \brief Makes the request a PUT request.
  ///
  /// The request body will be taken from the specified file starting from
  /// the given offset.
  virtual Status SetPutFromFile(const string& body_filepath, size_t offset,size_t size = 0);
 ...
 private:
  ...
  /// A header callback in the form which can be accepted by libcurl.
  static size_t PutCallback(void* ptr, size_t size, size_t nmemb,
                            FILE* this_object);
