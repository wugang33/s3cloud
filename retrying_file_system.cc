...
Status S3RetryingFileSystem::NewRandomAccessFile(
...
Status S3RetryingFileSystem::NewWritableFile(
...
Status S3RetryingFileSystem::NewAppendableFile(
    const string& filename, std::unique_ptr<WritableFile>* result) {
  std::unique_ptr<WritableFile> base_file;
...
Status S3RetryingFileSystem::NewReadOnlyMemoryRegionFromFile(
...
Status S3RetryingFileSystem::FileExists(const string& fname) {
  // No status -- no retries.
  return base_file_system_->FileExists(fname);
}

Status S3RetryingFileSystem::Stat(const string& fname, FileStatistics* stat) {
  return CallWithRetries(
      std::bind(&FileSystem::Stat, base_file_system_.get(), fname, stat),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::GetChildren(const string& dir,
...
Status S3RetryingFileSystem::GetMatchingPaths(const string& pattern,
                                            std::vector<string>* result) {
...
Status S3RetryingFileSystem::DeleteFile(const string& fname) {
  return CallWithRetries(
      std::bind(&FileSystem::DeleteFile, base_file_system_.get(), fname),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::CreateDir(const string& dirname) {
  return CallWithRetries(
      std::bind(&FileSystem::CreateDir, base_file_system_.get(), dirname),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::DeleteDir(const string& dirname) {
  return CallWithRetries(
      std::bind(&FileSystem::DeleteDir, base_file_system_.get(), dirname),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::GetFileSize(const string& fname, uint64* file_size) {
  return CallWithRetries(std::bind(&FileSystem::GetFileSize,
                                   base_file_system_.get(), fname, file_size),
                         initial_delay_microseconds_);
}

Status S3RetryingFileSystem::RenameFile(const string& src, const string& target) {
  return CallWithRetries(
      std::bind(&FileSystem::RenameFile, base_file_system_.get(), src, target),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::IsDirectory(const string& dirname) {
  return CallWithRetries(
      std::bind(&FileSystem::IsDirectory, base_file_system_.get(), dirname),
      initial_delay_microseconds_);
}

Status S3RetryingFileSystem::DeleteRecursively(const string& dirname,
