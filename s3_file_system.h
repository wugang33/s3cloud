...
#ifndef TENSORFLOW_CORE_PLATFORM_S3_FILE_SYSTEM_H_
#define TENSORFLOW_CORE_PLATFORM_S3_FILE_SYSTEM_H_
...
class S3FileSystem : public FileSystem {
 public:
  S3FileSystem();
  S3FileSystem(std::unique_ptr<AuthProvider> auth_provider,
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(S3FileSystem);
};

/// Google Cloud Storage implementation of a file system with retry on failures.
class RetryingS3FileSystem : public S3RetryingFileSystem {
 public:
 RetryingS3FileSystem()
   : S3RetryingFileSystem(std::unique_ptr<FileSystem>(new S3FileSystem)) {}
};
}
}  // namespace tensorflow
